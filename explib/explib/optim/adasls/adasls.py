...
class AdaSLS(torch.optim.Optimizer):
    def __init__(
        ...
        self.state["step"] = 0
        self.state["step_size_avg"] = 0.0
        self.beta = beta
        self.state["step_size"] = init_step_size
        self.step_size_max = 0.0
        ...
        if self.gv_option in ["scalar"]:
            ...
            if self.base_opt == "amsgrad":
                self.state["gv_max"] = [
                    torch.zeros(p.shape).to(p.device) for p in params
                ]
    ...
    def step(self, closure, clip_grad=False, logging=False):
        # increment step
        self.state["step"] += 1
        ...
        if self.gv_option in ["scalar"]:
            ...
            for i, g in enumerate(grad_current):
                if self.base_opt == "adagrad":
                    ...
                    self.state["gv"][i] = (1 - self.beta) * (g ** 2) + (
                        self.beta
                    ) * self.state["gv"][i]
        ...
        self.try_sgd_precond_update(
            self.params, step_size, params_current, grad_current, momentum=self.momentum
        )
        ...
        if self.pp_norm_method == "pp_armijo":
            ...
            for i, (g_i, gv_i) in enumerate(zip(grad_current, self.state["gv"])):
                if self.base_opt in ["diag_hessian", "diag_ggn_ex", "diag_ggn_mc"]:
                    ...
                    pv_i = 1.0 / (torch.sqrt(gv_i_scaled) + 1e-8)
    ...
    def get_step_size(
        ...
        if self.step_size_method == "sps":
            ...
            if self.adapt_flag in ["constant"]:
                ...
                coeff = self.gamma ** (1.0 / self.n_batches_per_epoch)
                # coeff = self.gamma
                if self.state["step"] == 1:
                    step_size = float(step_size)
                else:
                    step_size = min(coeff * self.state["step_size"], float(step_size))
            ...
            for e in range(100):
                ...
                found, step_size = self.check_armijo_precond_conditions(
                    step_size, loss, loss_next, grad_norm, pp_norm
                )
    ...
    def check_armijo_precond_conditions(
        self, step_size, loss, loss_next, grad_norm, pp_norm
    ):
        found = 0
    ...
    @torch.no_grad()
    def try_sgd_precond_update(
        self, params, step_size, params_current, grad_current, momentum
    ):
        if self.gv_option in ["scalar"]:
            zipped = zip(params, params_current, grad_current)

            for p_next, p_current, g_current in zipped:
                p_next.data = p_current - (step_size / torch.sqrt(gv)) * g_current
            ...
            if self.base_opt == "adam":
                ...
                for p_next, p_current, g_current, gv_i, mv_i in zipped:
                    gv_i_scaled = scale_vector(gv_i, self.beta, self.state["step"])
                    pv_list = 1.0 / (torch.sqrt(gv_i_scaled) + 1e-8)

                    if momentum == 0.0 or self.mom_type == "heavy_ball":
                ...
                for p_next, p_current, g_current, gv_i in zipped:
                    pv_list = 1.0 / (torch.sqrt(gv_i) + 1e-8)
                ...
                zipped = zip(params, params_current, grad_current, self.state["gv"])
                for p_next, p_current, g_current, gv_i in zipped:
                    pv_list = 1.0 / (gv_i + 1e-8)  # adding 1e-8 to avoid overflow.
                    # p_next.data = p_current - step_size * (pv_list *  g_current)
