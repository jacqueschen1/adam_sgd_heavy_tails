...
class LMOrderedIterator(object):
    ...
    def get_fixlen_iter(self, start=0):
        ...
        for i in range(start, end, self.bptt):
            if self.full_batch:
                seed = np.random.get_state()[1][0] % self.n_batch
                i += seed
                i = i % self.n_batch
...
def ptb_loader(
    batch_size, workspace_dir, device, tgt_len, drop_last=False, full_batch=False
):
    datadir = workspace_dir + "/datasets/penn/"
    ...
    if not os.path.isdir(datadir):
        subprocess.run(
            [
                "sh",
                "./get_ptb.sh",
                os.path.abspath(os.path.join(workspace_dir, "datasets")),
            ],
            check=True,
            cwd=cwd,
        )
...
def wikitext2_loader(
    ...
    datadir = os.path.join(workspace_dir, "datasets", "wikitext-2")
    cwd = os.path.dirname(os.path.realpath(__file__))
    if not os.path.isdir(datadir):
        subprocess.run(
            [
