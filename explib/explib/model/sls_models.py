...
"""Models from the SLS papers. Code source: https://github.com/IssamLaradji/sls"""


def get_model(model_name, model_args, train_set=None):
    ...
    if model_name == "mlp_dropout":
        ...
        if model_args is not None:
            init_diff = model_args["init_diff"]
            bigger_kernel = model_args["bigger_kernel"]
        else:
            init_diff = False
            bigger_kernel = False
        model = ResNet(
...
class ResNet(nn.Module):
    def __init__(
        ...
        block = block
        self.in_planes = 64
        self.bigger_kernel = bigger_kernel

        if self.bigger_kernel:
            self.conv1 = nn.Conv2d(
...
def DenseNet121(num_classes):
    return DenseNet(
        Bottleneck_DenseNet, [6, 12, 24, 16], growth_rate=32, num_classes=num_classes
    )
